...
deploy_to_env () {
    ...
    rsync -avz -e "ssh -o StrictHostKeyChecking=no" --delete wp-content/plugins/ ec2-user@13.239.37.152:/ecs/cp3402-${env}-storage/wp-content/plugins
    rsync -avz -e "ssh -o StrictHostKeyChecking=no" --delete wp-content/themes/arabusta/ ec2-user@13.239.37.152:/ecs/cp3402-${env}-storage/wp-content/themes/arabusta
}
